...
const addTypeConform = async () => {
  console.log(addTypeForm)
  let result = await reqAddCompType(addTypeForm)
  if (result.code == 200) {
    ElMessage({
      type: 'success',
      message: '添加类型成功',
...
const deleteType = async (row) => {
  console.log(row.competitiontypeid)
  let result = await reqDelCompType(row.competitiontypeid)
  if (result.code == 200) {
    ElMessage({
      type: 'success',
      message: '删除成功',
  ...
  ElMessageBox.confirm('确认删除这些类型吗?', 'Warning', {
    ...
    .then(async () => {
      ...
      for (let i = 0; i < selectChange.value.length; i++) {
        delId = selectChange.value[i].competitiontypeid
        await reqDelCompType(delId)
      }
      await getCompType()
...
const editType = async (row) => {
  ...
  console.log(row)
  editTypeForm.competitiontypeid = row.competitiontypeid
...
const editConform = async () => {
  ...
  let result = await reqUpdateCompType(editTypeForm)
  if (result.code == 200) {
    editTypeVisible.value = false
    await getCompType()
  }
