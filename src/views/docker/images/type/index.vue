...
<template>
  ...
  >
    <div style="font-size: 18px">镜像类型</div>
  ...
  <el-pagination
    ...
    :total="imageStore.imageTypeTotal"
    @current-change="currentChange"
    @size-change="sizeChange"
  />
  <el-dialog v-model="addTypeVisible" title="添加容器镜像" draggable overflow>
    <el-form style="margin-top: 40px; margin-left: 180px" label-width="100px">
      <el-form-item label="镜像类型名称">
        <el-input
          style="width: 250px"
      ...
      <el-form-item label="镜像编码" style="margin-top: 30px">
        <el-input
          style="width: 250px"
      ...
      <el-form-item label="镜像描述" style="margin-top: 30px">
        <el-input
          placeholder="请输入类型描述"
...
const addTypeConform = async () => {
  let result = await reqAddImageType(addTypeForm)
  if (result.code == 200) {
    ElMessage({
      type: 'success',
      message: '添加类型成功',
...
const deleteType = async (row: imageStore.imageType) => {
  let result = await reqDelImageType(row.imagetypeid)
  if (result.code == 200) {
    ElMessage({
      type: 'success',
      message: '删除成功',
  ...
  ElMessageBox.confirm('确认删除这些类型吗?', 'Warning', {
    ...
    .then(async () => {
      ...
      for (let i = 0; i < selectChange.value.length; i++) {
        delId = selectChange.value[i].imagetypeid
        await reqDelImageType(delId)
      }
      await imageStore.GetImageTypeList(pageCurrent.value, pageSize.value)
...
const editType = async (row) => {
  ...
  editTypeForm.imagetypeid = row.imagetypeid
...
const editConform = async () => {
  let result = await reqUpdateImageType(editTypeForm)
  editTypeVisible.value = false
  await imageStore.GetImageTypeList(pageCurrent.value, pageSize.value)
  console.log(result)
}
